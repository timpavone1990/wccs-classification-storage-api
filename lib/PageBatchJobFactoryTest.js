...
describe("#createPageStorageJob", () => {
    it("should compute the statements correctly", () => {
        const statements = pageBatchJobFactory.createPageStorageJob(testPage);
        expect(statements).to.eql({
            "statements": [{
                "statement":
                    ...
                    "WITH p\n" +
                    "OPTIONAL MATCH (p)-[e:Property]->()\n" +
                    "DELETE e\n" +
                    "MERGE (p_0:Content {type: $p_0.type})\n" +
                    "MERGE (p_1_0:Content {type: $p_1_0.type})\n" +
                    "MERGE (p_1_1:Content {type: $p_1_1.type})\n" +
                    "MERGE (p_1:Content {type: $p_1.type})-[:Property]->(p_1_0)-[:Parent]->(p_1)-[:Property]->(p_1_1)-[:Parent]->(p_1)\n" +
                    "MERGE (p)-[:Property]->(p_0)\n" +
                    "MERGE (p)-[:Property]->(p_1)\n" +
                    "WITH p\n" +
                    "MATCH (c:Content) WHERE NOT (:Page)-[:Property*]->(c)\n" +
                    "DETACH DELETE c\n" +
                    "SET p.type = $p.type\n"
                    ),
                "parameters": {
                    "p_0": {"type": "MyProperty1Type"},
                    "p_1_0": {"type": "MyProperty2.1Type"},
                    "p_1_1": {"type": "MyProperty2.2Type"},
                    "p_1": {"type": "MyProperty2Type"},
