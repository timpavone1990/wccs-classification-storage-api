...
class PageStatementBuilder {
    ...
    static build(page, site) {
        ...
        const statement =
            ...
            builder.site(siteAlias, pageAlias) + "\n" +
            builder.deleteObsoleteSiteToPageRelationships(siteAlias, pageAlias) + "\n" +
        ...
        propertyNames.forEach((propertyName, pagePropertyIndex) => {
            ...
            if (Array.isArray(property)) {
                property.forEach((item, propertyItemIndex) => {
                    statements.push(this.propertyRelationship(objectAlias, `${objectAlias}_prop${pagePropertyIndex}_item${propertyItemIndex}`));
                });
            } else {
                statements.push(this.propertyRelationship(objectAlias, `${objectAlias}_prop${pagePropertyIndex}`));
            }
        });
        return statements.join("\n");
    }
    ...
    page(pageAlias) {
        ...
        return `MERGE (${pageAlias}:Page {url: $${pageAlias}.url})\n` +
    ...
    site(siteAlias, pageAlias) {
        const siteToPageRelationshipAlias = "s_page0";
        this.nodeAliases.add(siteAlias);
        this.nodeAliases.add(siteToPageRelationshipAlias);
        return `MERGE (${siteAlias}:Site {name: $s.name})\n` +
            `MERGE (${siteAlias})-[${siteToPageRelationshipAlias}:Owns]->(${pageAlias})`;
    }

    deleteObsoleteSiteToPageRelationships(siteAlias, pageAlias) {
