...
module.exports.createPageStorageJob = (page) => {
    ...
    statement += `WITH ${pageAlias}` + "\n";
    statement += `OPTIONAL MATCH (${pageAlias})-[e:Property]->()` + "\n";
    statement += "DELETE e\n";
    ...
    if (page.properties) {
        ...
        for (let i = 0; i < propertyNames.length; i++) {
            if (!Array.isArray(page.properties[propertyNames[i]])) {
                ...
                statement += `MERGE (${pageAlias})-[:Property]->(${childPropertyAlias})` + "\n";
            }
        }

        statement += `WITH ${pageAlias}` + "\n";
        statement += "MATCH (c:Content) WHERE NOT (:Page)-[:Property*]->(c)\n"
        statement += "DETACH DELETE c\n"
    } else {
        statement += `MERGE (${pageAlias}:Page {url: $${pageAlias}.url})` + "\n";
    }
