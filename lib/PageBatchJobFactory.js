...
class PageStatementBuilder {
    ...
    referencedResource(reference, name, referenceAlias) {
        let statements = [];
        if (Array.isArray(reference)) {
            statements = reference.map((reference, index) => {
                return this.referencedResource(reference, name, `${referenceAlias}_item${index}`);
            });
        } else {
        ...
        propertyNames.forEach((propertyName, index) => {
            ...
            if (Array.isArray(property)) {
                property.forEach((item, propertyItemIndex) => {
                    this.pushIfNotBlank(statements, this.propertyRelationship(objectAlias, `${objectAlias}_prop${index}_item${propertyItemIndex}`, true));
                });
            } else {
                this.pushIfNotBlank(statements, this.propertyRelationship(objectAlias, `${objectAlias}_prop${index}`, false));
            }
        });
        return statements.join("\n");
    }

    propertyRelationship(objectAlias, propertyAlias, isCollectionProperty) {
        const label = isCollectionProperty ? "OwnsMany" : "Owns";
        return `MERGE (${objectAlias})-[:${label} {${this.propertyRelationshipProperties(propertyAlias)}}]->(${propertyAlias})`;
    }
        ...
        referenceNames.forEach((referenceName, index) => {
            ...
            if (Array.isArray(reference)) {
                reference.forEach((item, referenceItemIndex) => {
                    ...
                    const referencedResourceAlias = `${referenceAlias}_r`;
                    this.pushIfNotBlank(statements, `MERGE (${objectAlias})-[${this.referenceRelationship(referenceAlias, true)}]->(${referencedResourceAlias})`);
                });
            } else {
                const referenceAlias = `${objectAlias}_ref${index}`;
                const referencedResourceAlias = `${referenceAlias}_r`;
                this.pushIfNotBlank(statements, `MERGE (${objectAlias})-[${this.referenceRelationship(referenceAlias, false)}]->(${referencedResourceAlias})`);
            }
        });
        return statements.join("\n");
    }

    referenceRelationship(referenceAlias, isCollectionReference) {
        const label = isCollectionReference ? "ReferencesMany" : "References";
        this.nodeAliases.add(referenceAlias);
        return `${referenceAlias}:${label} {${this.referenceRelationshipProperties(referenceAlias)}}`;
    }

    referenceRelationshipProperties(referenceAlias) {
        return `name: $${referenceAlias}.name,type: $${referenceAlias}.type,${this.selectorProperties(referenceAlias)}`;
    }
