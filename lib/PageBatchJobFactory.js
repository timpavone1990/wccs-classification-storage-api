...
class PageStatementBuilder {
    ...
    static build(page, site) {
        ...
        const statement =
            ...
            `WHERE q IN [${Array.from(builder.aliases).join(",")}] AND NOT deletedProperty IN [${Array.from(builder.aliases).join(",")}]\n` +
            ...
            `WHERE x IN [${Array.from(builder.aliases).join(",")}] AND NOT y IN [${Array.from(builder.aliases).join(",")}]\n` +
