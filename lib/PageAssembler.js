...
module.exports = class PageAssembler {
    ...
    handleProperty(propertiesByChecksum, propertyDef, parent, propertyReferencesByChecksum, subPropertiesByChecksum, visited) {
        ...
        cleanProperty.class = propertiesByChecksum[propertyDef.checksum].class;
        ...
        if (visited.hasOwnProperty(propertyDef.checksum)) {
            ...
            cleanProperty.contents = visited[propertyDef.checksum].contents;
            cleanProperty.references = visited[propertyDef.checksum].references;
        } else {
            visited[propertyDef.checksum] = cleanProperty;

            if (!cleanProperty.contents) {
                cleanProperty.contents = {};
            }
        ...
        if (cleanProperty.isCollection) {
            if (!parent.contents[cleanProperty.name]) {
                parent.contents[cleanProperty.name] = [];
            }
            parent.contents[cleanProperty.name].push(cleanProperty);
        } else {
            parent.contents[cleanProperty.name] = cleanProperty;
        }
